...
# 训练集上的情况也会在测试集上出现
"""
- freelzy大牛对于出来trick数据的方式没有CharlesXu那么简洁方便
- freelzy大牛对于trick数据的处理方式bettenW的处理方式很类似，代码也几乎一样，但感觉bettenW处理的更为细致。https://github.com/bettenW/IJCAI18_Tianchi_Rank29
- freelzy大牛的牛逼之处在于，用了更高阶的feature来处理trick数据
"""
def doTrick(data):
...
"""
- 只是基于一阶特征userID进行trick处理，和高阶的trick处理有重复。
- 二者的区别在于数据粒度不一样。
- 和doTrick的output可能存在多重共线性，所以才导致线上效果不好。
"""
...
# 均值特征
"""
新学trick: 快速添加统计值特征的方法（感觉在20th那有里有总结过，但忘记写在哪里了 - - ）
    - 选取统计特征feature_a，例如hour
    - 对各个维度进行group_by
    - 设置算子，比如mean()
    - 进行.reset_index()，重置列名，left_join
"""
...
"""
新学trick：活跃数特征的定义
    - 活跃数特征的命名：dimension_active_feature
    - 命名含义：在dimension维度下，feature的活跃程度，活跃程度的计算基于nunique()进行
    - 玩转方法还是基于聚合计算。
"""
def doActive(X_train, X_test):
    ...
    """
    复习一下：
        - nunique 只作用于 Series, 用法是 Series.nunique()，返回 Series 中只出现过一次的元素，类似set()运算
        - data.groupby(["userID"]).hour 是一个Series
        - data.groupby(["userID"])[['hour']] 是一个DataFrame
    """
...
"""
新学trick：添加交叉特征，即求笛卡尔积后进行label_encode
"""
...
"""
- 这部分所需要的中间数据已经在doFeaus_1.py中计算好了
- 这里只是在做平滑处理
"""
# 一阶特征CTR的平滑处理
...
# 二阶特征CTR的平滑处理，userID和positionID的点击次数重要性排名靠前，所有统计特征只加了这一个点击次数
...
# 三阶特征CTR的平滑处理，三特征组合从周冠军分享的下载行为和网络条件限制，以及用户属性对app需求挖掘出
