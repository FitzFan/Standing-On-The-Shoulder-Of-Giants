...
前N天的比率：
    ...
    - 求和比率
trick总结：
    - 构造特征就是不同的特征或者特征组合（低阶或高阶），进行group_by并施加合适的算子
    - 然后和原始的data_frame进行left_join
    - 复杂点的可能会求个比率，会做一下归一化，会进行一些分桶，blabla
"""
...
def doNew(data):
    ...
    """
    复习一下：
        - 1*(temp['large2']>2) 是一个逻辑判断
        - 对于temp['large2']>2，值为1，否则值为0
    """
